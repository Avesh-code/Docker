...
## 7. Docker volume and storage 
* Docker volumes are like special folders on your computer where Docker stores data for containers. This way, even if a container stops or is removed, your data stays safe.
* To list the volumes
```
docker volume ls
```
* to create a new volumes
```
docker volume create mysql-data
```
* we can also see the information about volume
```
docker inspect mysql-data
```
* as we have creted the volume but we have to also attach the volume to its container so that the data can be stored there.
```
# for mapping we use -v for volumes Example:
...
# for making our own directory and want to keep all the data there we first create the directory and then attach that directory to container . example
docker run -d --name mysql --network two-tier -v /home/aveshpadaya/docker/volumes/mysql:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=devops mysql
# here the /home/aveshpadaya/docker/volumes/mysql is volume made by me and i have to use it as volume. 
```
* Note : So if we remove the container and want to attach volume to new container we can specify that volume to that container.
